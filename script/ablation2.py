...
# -*- coding: utf-8 -*-
"""
ablation2_mask_and_l2.py  （CirCor / herat2.py）

两个消融实验（基于 CirCor 数据 + herat2.py 的 subject-level 9:1 split）：

1) 频带硬 mask：
   - 只保留低频 (band1+2) / 高频 (band3+4)，评估 CE-60 / SCA-long / SCA-all。
   - 使用 herat2.py 训练好的：
       pcg_resnet18_melspec_ce_long_best2.pt
       pcg_resnet18_melspec_sca_long_best2.pt
       pcg_resnet18_melspec_sca_all_best2.pt

2) 去掉 SCA，仅对 penultimate feature 做 L2 正则 (CE + λ‖h‖²)，从头训练 60 epoch，
   在 λ ∈ {0.05, 0.1, 0.2, 0.3, 0.5} 上做网格搜索（同样用 CirCor 的 train/val split）。
"""
...
import herat2 as heart  # 注意这里是 herat2.py
...
# 复用 herat2.py 里的 band 切分
band_slices = heart.band_slices  # e.g. [(0,32),(32,64),(64,96),(96,128)]
...
# CirCor 上训练好的 3 个模型（subject-level split）
CE_LONG_CKPT = heart.CE_LONG_BEST_PATH2
SCA_LONG_CKPT = heart.SCA_LONG_BEST_PATH2
SCA_ALL_CKPT = heart.SCA_ALL_BEST_PATH2
...
def run_mask_ablation():
    ...
    print("\n================ MASK ABLATION (low / high bands, CirCor) ================")
    ...
    # 加载三种模型（CirCor 对应的 *2 权重）
    ce_model = PCGResNet18(n_classes=2).to(device)
    ce_model.load_state_dict(torch.load(CE_LONG_CKPT, map_location=device))

    sca_long_model = PCGResNet18(n_classes=2).to(device)
    sca_long_model.load_state_dict(torch.load(SCA_LONG_CKPT, map_location=device))

    sca_all_model = PCGResNet18(n_classes=2).to(device)
    sca_all_model.load_state_dict(torch.load(SCA_ALL_CKPT, map_location=device))
...
L2_ALL_BEST_PATH = "pcg_resnet18_melspec_l2feat_all_best2.pt"
...
def run_feature_l2_all(
    ...
    """
    从随机初始化开始，用 CE + λ‖h‖² 训练 60 epoch（CirCor subject-level split），
    ...
    print("\n================ FEATURE L2 ABLATION (CE + λ‖h‖², CirCor) ================")
