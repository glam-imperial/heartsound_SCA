"""
analyze_sca_prior_alignment.py

比较 CE-60, SCA-long, SCA-all 在 “频率启发的 Mel-band prior” 下的解释对齐情况：
- 都在同一个 validation 集合上
- 使用 Grad×Input + band 聚合得到 P_attr

Step 0: 收集三种模型在 val 上的 band-level attribution 分布
...
import herat as heart   # 确保文件名是 herat.py
...
def collect_attr_data(batch_size: int = 4):
    """
    在 validation 集合上，分别用 CE-60, SCA-long, SCA-all 模型计算 Grad×Input attribution，
    并按 mel-bands 聚合成 P_attr，返回一个 dict 用于后续所有分析。
    ...
    # 使用 herat 里的 val_ds + collate_mel_batch 构建一个“小 batch”的 val_loader
    ...
    build_prior = heart.build_toy_prior   # 现在内部实际是频率启发 prior

    # 直接从 herat 里读 checkpoint 路径，避免写死
    CE_LONG_BEST_PATH      = heart.CE_LONG_BEST_PATH
    SCA_LONG_BEST_PATH     = heart.SCA_LONG_BEST_PATH
    SCA_ALL_LONG_BEST_PATH = heart.SCA_ALL_LONG_BEST_PATH
...
def summarize_prior_alignment(data):
    ...
    # 从 herat.py 读 freq-informed prior（防止未来改权重）
...
if __name__ == "__main__":
    # Step 0: 在 val 上跑一遍，收集 CE-60 / SCA-long / SCA-all attribution 数据
